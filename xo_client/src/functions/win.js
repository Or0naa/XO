...
export function check(arr, value) {
    if (row(arr, value, 0, 0) || colomn(arr, value, 0, 0) ||
        value == arr[0, 0] && diagonaldown(arr, value, 0, 0) || value == arr[arr.length - 1, 0] && diagonalup(arr, value, arr.length - 1, 0)) {
        alert("You Win");
        return true;
    }
    else return false;
